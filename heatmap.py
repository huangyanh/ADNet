...
class yolov8_target(torch.nn.Module):
    def __init__(self, ouput_type, conf, ratio, target_class=None) -> None:
        super().__init__()
        self.ouput_type = ouput_type
        self.conf = conf
        self.ratio = ratio
        self.target_class = target_class  # 新增：指定目标类别
    ...
    def forward(self, data):
        ...
        for i in trange(int(post_result.size(0) * self.ratio)):
            ...
            if self.ouput_type == 'class' or self.ouput_type == 'all':
                if self.target_class is None or post_result[i].argmax() == self.target_class:  # 仅考虑目标类别
...
class yolov11_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize, target_class=None):
        ...
        target = yolov8_target(backward_type, conf_threshold, ratio, target_class)  # 传递 target_class
    ...
    def process(self, img_path, save_path):
        ...
        try:
            ...
            print(f"Error generating grayscale cam: {e}")
            return
    ...
    def __call__(self, img_path, save_path):
        print(f"Calling __call__ with img_path={img_path}, save_path={save_path}")  # 调试信息
...
def get_params():
    params = {
        'weight': 'runs/train/Mobile_MonteCarlo/weights/best.pt',  # 只需要指定权重即可
        'device': 'cuda:0',
        ...
        'conf_threshold': 0.7,  # 0.2
        'ratio': 0.02,  # 0.02-0.1
        'show_box': False,
        'renormalize': True,
        'target_class': 2  # 新增：指定目标类别，例如类别0
    }
    return params

if __name__ == '__main__':
    model = yolov11_heatmap(**get_params())
    model(r'Adv_test/192.168.0.64_01_2020090812501514.jpg', 'Adv_test/result')